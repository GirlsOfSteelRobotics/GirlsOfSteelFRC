...
class WidgetGenerator:
    ...
    def __init__(self, template_dir, project_dir, overall_package, widget_config):
        ...
        self.gen_src_dir = os.path.abspath(os.path.join(project_dir, "src", "dashboard_gen", "java"))
        ...
        self.project_package_src_dir = os.path.join(self.src_dir, self.project_package_name_as_rel_dir)
        self.widget_package_src_dir = os.path.join(self.src_dir, self.widget_package_name_as_rel_dir)

        self.widget_package_gen_dir = os.path.join(self.gen_src_dir, self.widget_package_name_as_rel_dir)
        self.widget_resource_dir = os.path.join(self.resource_dir, self.widget_package_name_as_rel_dir)
    ...
    def dump_single_components(self):
        ...
        for child in sorted(self.widget_config['children_tables'], key=lambda x: 1):
            ...
            data_type_path = os.path.join(self.widget_package_gen_dir, child["table"] + "DataType.java")
    ...
    def dump_widget(self):
        ...
        kwargs = self.default_kwargs.copy()
        template_output = self._load_template('dashboard_gen/widget/widget_template.txt').render(**kwargs)

        output_file = os.path.join(self.widget_package_gen_dir, self.widget_config['table'] + "Widget.java")
        open(output_file, 'w').write(template_output)

    def dump_widget_top_level_data(self):
        if len(self.widget_config['children_tables']) == 1:
            return
        data_template_output = self._load_template('dashboard_gen/widget/parent_data_template.txt').render(**self.default_kwargs)
        data_type_template_output = self._load_template('dashboard_gen/widget/parent_data_type_template.txt').render(**self.default_kwargs)

        data_output_file = os.path.join(self.widget_package_gen_dir, self.widget_config['table'] + "Data.java")
        data_type_output_file = os.path.join(self.widget_package_gen_dir, self.widget_config['table'] + "DataType.java")
    ...
    def maybe_dump_fxml(self, force):
        ...
        if force or not os.path.exists(widget_file):
            ...
            template_output = self._load_template('resources/fxml_widget_template.txt').render(kwargs)
        ...
        if force or not os.path.exists(controller_file):
            ...
            template_output = self._load_template('resources/fxml_controller_template.txt').render(kwargs)
            open(controller_file, 'w').write(template_output)

    def maybe_dump_standalone_main(self, force):
        output_file = os.path.join(self.widget_package_src_dir, self.widget_config['table'] + "StandaloneMain.java")

        kwargs = self.default_kwargs.copy()
        if force or not os.path.exists(output_file):
            print("Create optional file " + output_file)
            open(output_file, 'w').write(self._load_template('java/standalone_main_template.txt').render(**kwargs))

    def maybe_dump_controller(self, force):
        kwargs = self.default_kwargs.copy()
        template_output = self._load_template('java/widget/controller_template.txt').render(**kwargs)

        output_file = os.path.join(self.widget_package_src_dir, self.widget_config['table'] + "Controller.java")
        if force or not os.path.exists(output_file):
            print("Create optional file " + output_file)
            open(output_file, 'w').write(template_output)

    def maybe_dump_shuffleboard_names(self, force):
        template_output = self._load_template('java/widget/shuffleboard_names_template.txt').render(**self.default_kwargs.copy())
...
class TopLevelGenerator:
    ...
    def dump_plugin(self):
        ...
        template_output = self._load_template('dashboard_gen/plugin_template.txt').render(overall_config=self.overall_config)

        output_file = os.path.join(self.plugin_dump_gen_dir, self.overall_config['plugin_name'] + ".java")
        open(output_file, 'w').write(template_output)
...
def maybe_add_standalone_buttons(widget_config):
    ...
    double_keys = [('Q', 'A'), ('W', 'S'), ('E', 'D'), ('R', 'F'), ('T', 'G'), ('Y', 'H'), ('U', 'J'), ('I', 'K')]
