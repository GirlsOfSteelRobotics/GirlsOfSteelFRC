...
public class Alert implements AutoCloseable {
    ...
    private static final class SendableAlerts implements Sendable {
        ...
        public Set<PublishedAlert> getActiveAlertsStorage(AlertType type) {
            return m_alerts.computeIfAbsent(type, ttype -> new TreeSet<>());
        }
        ...
        private static SendableAlerts forGroup(String group) {
            return GROUPS.computeIfAbsent(
                ...
                ggroup -> {
                    ...
                    SmartDashboard.putData(ggroup, sendable);
                    return sendable;
                });
        }
    }
    ...
    public static boolean hasErrors() {
        for (SendableAlerts sendableAlerts : SendableAlerts.GROUPS.values()) {
            if (!sendableAlerts.getActiveAlertsStorage(AlertType.ERROR).isEmpty()) {
                return true;
            }
        }
        return false;
    ...
    public static boolean hasWarnings() {
        for (SendableAlerts sendableAlerts : SendableAlerts.GROUPS.values()) {
            if (!sendableAlerts.getActiveAlertsStorage(AlertType.WARNING).isEmpty()) {
                return true;
            }
        }
        return false;
