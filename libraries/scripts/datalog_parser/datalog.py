...
class DataLogRecord:
    ...
    def isStart(self) -> bool:
        return self.entry == 0 and len(self.data) >= 17 and self._getControlType() == kControlStart

    def isFinish(self) -> bool:
        return self.entry == 0 and len(self.data) == 5 and self._getControlType() == kControlFinish
...
class DataLogIterator:
    ...
    def __next__(self) -> DataLogRecord:
        ...
        record = DataLogRecord(
            entry, timestamp, self.buf[self.pos + headerLen : self.pos + headerLen + size]
        )
...
class DataLogReader:
    ...
    def isValid(self) -> bool:
        ...
        return len(self.buf) >= 12 and self.buf[0:6] == b"WPILOG" and self.getVersion() >= 0x0100
