package com.gos.reefscape;

import edu.wpi.first.math.geometry.Pose2d;
import edu.wpi.first.math.geometry.Rotation2d;

public class ChoreoPoses {
    public static final Pose2d A = new Pose2d(3.237511157989502, 4.223461151123047, Rotation2d.fromRadians(3.141592653589793));
    public static final Pose2d AB = new Pose2d(3.257780075073242, 4.0005035400390625, Rotation2d.fromRadians(3.141592653589793));
    public static final Pose2d B = new Pose2d(3.257780075073242, 3.83835244178772, Rotation2d.fromRadians(3.141592653589793));
    public static final Pose2d C = new Pose2d(3.606261730194092, 3.079380989074707, Rotation2d.fromRadians(-2.1662063752267593));
    public static final Pose2d CD = new Pose2d(3.8455777168273926, 2.926252603530884, Rotation2d.fromRadians(-2.073639831228008));
    public static final Pose2d D = new Pose2d(4.0077290534973145, 2.8249082565307617, Rotation2d.fromRadians(-2.073639831228008));
    public static final Pose2d E = new Pose2d(4.955280303955078, 2.8178365230560303, Rotation2d.fromRadians(-1.0471975511965976));
    public static final Pose2d EF = new Pose2d(5.147095680236816, 2.9247190952301025, Rotation2d.fromRadians(-1.0516503687020464));
    public static final Pose2d F = new Pose2d(5.345475196838379, 3.0681347846984863, Rotation2d.fromRadians(-1.0040661405713345));
    public static final Pose2d G = new Pose2d(5.771121978759766, 3.777545928955078, Rotation2d.fromRadians(0.0));
    public static final Pose2d GH = new Pose2d(5.739912986755371, 3.987903594970703, Rotation2d.fromRadians(0.0));
    public static final Pose2d H = new Pose2d(5.739912986755371, 4.194386005401611, Rotation2d.fromRadians(0.0));
    public static final Pose2d HUMAN_PLAYER_LEFT = new Pose2d(1.3718515634536743, 7.273766994476318, Rotation2d.fromRadians(2.279422598922567));
    public static final Pose2d HUMAN_PLAYER_RIGHT = new Pose2d(1.5276724100112915, 0.642888605594635, Rotation2d.fromRadians(-2.248504688139378));
    public static final Pose2d I = new Pose2d(5.345475196838379, 5.0139479637146, Rotation2d.fromRadians(1.0460000182544436));
    public static final Pose2d IJ = new Pose2d(5.142786502838135, 5.115292549133301, Rotation2d.fromRadians(1.0460000182544436));
    public static final Pose2d J = new Pose2d(4.943519115447998, 5.226694107055664, Rotation2d.fromRadians(1.0370881817864992));
    public static final Pose2d K = new Pose2d(4.048266887664795, 5.257174491882324, Rotation2d.fromRadians(2.077895386035358));
    public static final Pose2d KL = new Pose2d(3.865846633911133, 5.155829906463623, Rotation2d.fromRadians(2.077895386035358));
    public static final Pose2d L = new Pose2d(3.618795394897461, 4.982372760772705, Rotation2d.fromRadians(2.095591826496163));
    public static final Pose2d PROCESSOR = new Pose2d(6.35289192199707, 0.45048725605010986, Rotation2d.fromRadians(1.5707963267948966));
    public static final Pose2d STARTING_POS_CENTER = new Pose2d(7.832268238067627, 4.001668930053711, Rotation2d.fromRadians(0.0));
    public static final Pose2d STARTING_POS_LEFT = new Pose2d(7.84056282043457, 6.1349568367004395, Rotation2d.fromRadians(0.0));
    public static final Pose2d STARTING_POS_RIGHT = new Pose2d(7.901095867156982, 1.8680174350738523, Rotation2d.fromRadians(0.0));

}
