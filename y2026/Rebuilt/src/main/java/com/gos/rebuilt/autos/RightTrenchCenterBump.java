...
public class RightTrenchCenterBump extends GosAuto {
    public RightTrenchCenterBump(ChassisSubsystem chassisSubsystem, CombinedCommand combinedCommand) {
        super(StartingPositions.LEFT, List.of(AutoActions.CROSSTHETRENCH, AutoActions.SHOOOT, AutoActions.CROSSBUMP));

        addCommands(Commands.runOnce(() -> chassisSubsystem.resetPose(ChoreoUtils.getPathStartingPose("RightTrenchCenterBump").getPose())));
        addCommands(new ParallelDeadlineGroup(followChoreoPath("RightTrenchCenterBump"), combinedCommand.intake()));
