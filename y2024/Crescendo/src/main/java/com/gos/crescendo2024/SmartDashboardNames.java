package com.gos.crescendo2024;

public final class SmartDashboardNames {

    public static final String WIDGET_NAME = "Crescendo Bot Super Structure Widget";

    public static final String SUPER_STRUCTURE_TABLE_NAME = "SuperStructureTableName";

    public static final String SUPER_STRUCTURE = "SuperStructure";
    public static final String PIVOT_MOTOR_ANGLE                = "PivotMotorAngle";
    public static final String GOAL_ANGLE                       = "GoalAngle";
    public static final String SHOOTER_MOTOR_PERCENTAGE         = "ShooterMotorPercentage";
    public static final String PIVOT_MOTOR_PERCENTAGE           = "PivotMotorPercentage";
    public static final String HAS_GAME_PIECE                   = "HasGamePiece";
    public static final String INTAKE_MOTOR_PERCENTAGE          = "IntakeMotorPercentage";


    private SmartDashboardNames() {

    }
}
